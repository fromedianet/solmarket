...
const CANDY_INSTR = Buffer.from(sha256.digest('global:claim_candy')).slice(
  0,
  8,
);
const EDITION_INSTR = Buffer.from(sha256.digest('global:claim_edition')).slice(
  0,
  8,
);
const SIGNER = Keypair.fromSecretKey(
  Buffer.from(JSON.parse(process.env.MA_SIGNER)),
);
...
const verifyOTP = async event => {
  ...
  const response = {
    statusCode: 200,
    body: JSON.stringify(
      bs58.encode(nacl.sign.detached(stored.TXN, SIGNER.secretKey)),
    ),
  };
  return response;
};
